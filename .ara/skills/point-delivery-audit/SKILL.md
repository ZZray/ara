...
5. Run an independent scoped code review for non-trivial or shared behavior. Record the reviewer/tool, exact diff scope, findings, and resolutions; when a reviewer is unavailable, record the gap rather than claiming independent audit. Then inspect ownership, permissions, cancellation, unknown effects, credentials, and regression risk yourself. Re-run affected tests after fixes.
6. Mark `accepted` only when mandatory execution and audit evidence passes on the delivered snapshot. Otherwise use `changes requested` or `not tested` and keep the ledger/marker truthful. A successful Run is not Task acceptance. This Skill does not automatically invoke a reviewer.
