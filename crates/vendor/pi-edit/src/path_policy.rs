...
impl PathPolicy {
	...
	pub fn recover_missing(&self, authored: &str) -> Option<Resolved> {
		...
		// ARA: `ara-walk` (the port of pi-walker's ignore-state walk) replaces
		// `pi_walker::WalkRequest`; same options, glob and two-entry limit.
		let glob = globset::GlobBuilder::new(&format!("**/{escaped}"))
			.literal_separator(true)
			.build()
			.ok()?
			.compile_matcher();
		let budget = ara_walk::Budget {
			cancel:   Default::default(),
			deadline: Instant::now() + Duration::from_secs(5),
		};
		let options = ara_walk::WalkOptions {
			include_hidden:    true,
			use_gitignore:     true,
		...
		ara_walk::walk(&self.cwd, options, &budget, |entry| {
			if glob.is_match(&entry.relative) {
				found.push(entry.relative);
				if found.len() >= 2 {
					return ara_walk::Visit::Stop;
				}
			}
			ara_walk::Visit::Continue
		})
		.ok()?;
		if found.len() != 1 {
			return None;
		}
		let display = found.pop()?;
