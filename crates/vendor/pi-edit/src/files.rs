...
pub struct FileRead {
	...
	/// ARA: the bytes were not valid UTF-8 and `raw` is a lossy decode.
	pub lossy:       bool,
}
...
impl FileRead {
	...
	pub fn persist(&self, after_lf: &str) -> EditResult<String> {
		// ARA: re-encoding a lossy decode would write U+FFFD over every
		// undecodable byte of the file; refuse while staging (nothing written).
		if self.lossy {
			return Err(EditError::apply(format!(
				"Refusing to edit {}: the file is not valid UTF-8 text, and editing it would replace \
				 its undecodable bytes. Use `write` with the complete intended content, or `bash`.",
