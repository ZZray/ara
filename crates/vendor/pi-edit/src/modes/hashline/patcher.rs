...
pub fn stage_patch(
	...
	// ARA: files this payload removes, in payload order (see the MV guard).
	let mut removed = std::collections::HashSet::<std::path::PathBuf>::new();
	for original in &patch.sections {
		...
		if let Some(FileOp::Move { dest }) = &parsed.file_op {
			...
			let dest_canonical = crate::path_policy::canonical_key(&destination.absolute);
			if dest_canonical == read.canonical {
				return Err(EditError::apply(format!(
					"MV destination is the same as {}.",
					section.path
				)));
			}
			// ARA: never overwrite an existing file (upstream writes over it);
			// same wording as patch mode's rename guard. A destination this
			// payload removed earlier (`REM`) is free.
			if files.exists(&destination.absolute) && !removed.contains(&dest_canonical) {
				return Err(EditError::apply(format!(
					"Cannot move {} to {dest}: destination already exists.",
					section.path
				)));
			}
		}
		if matches!(parsed.file_op, Some(FileOp::Rem)) {
			removed.insert(read.canonical.clone());
		}
